...
//        test.applyLinearImpulse(new Vector2(-.5f, 0), test.getPosition(), true);
        ...
        entityWorld.getAspectSubscriptionManager().get(Aspect.all(Physics.class)).addSubscriptionListener(new EntitySubscription.SubscriptionListener() {
            @Override
            ...
            public void removed(IntBag entities) {
                for (int ent : entities.getData()) {
                    Body b = entityWorld.getMapper(Physics.class).get(ent).body;
                    if(b != null){
                        physicWorld.destroyBody(b);
                        entityWorld.getMapper(Physics.class).get(ent).body = null;
                    }
                }
            }
        });
...
//        Matrix4 m = new Matrix4().rotate(Vector3.X, rot);
//        vec.mul(m);
//        camera.translate(0, -amount*vec.y, amount*vec.z);
